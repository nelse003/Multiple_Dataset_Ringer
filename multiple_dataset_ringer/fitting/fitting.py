...
def three_gaussian_offset(
    x, a1, x01, sigma_1, a2, x02, sigma_2, a3, x03, sigma_3, offset
):

    """Three gaussian curve with offset (10 parameters)"""

    return (
        single_gaussian(x, a1, x01, sigma_1)
...
def three_gaussian_fix(x, a1, sigma_1, a2, sigma_2, a3, sigma_3, offset):
    ...
    Has 7 Parameters
    """
    return three_gausian_offset(
        x, a1, 60, sigma_1, a2, 180, sigma_2, a3, 300, sigma_3, offset
    )
...
def three_normal_fix(x, sigma_1, sigma_2, sigma_3, offset):
    ...
    return three_gaussian_fix(
        x, amplitude, sigma_1, amplitude, sigma_2, amplitude, sigma_3, offset
    )
    ...
    for residue, data in ref_set.iterrows():
        ...
        if not os.path.exists(os.path.join(out_dir, residue, parameters_csv_filename)):
            ...
            interpolate_csv = "{}_{}_Datasets_{}_{}-ringer.csv".format(
                residue, len(datasets), map_type, angle_type
            )
            ...
            for dataset in interpolated_results.index:
                ...
                fit_parameters.to_csv(
                    os.path.join(out_dir, residue, parameters_csv_filename)
                )
...
def generate_RMSD(
    ...
    if not os.path.exists(os.path.join(out_dir, RMSD_filename)):
        for residue, data in ref_set.iterrows():
            ...
            interpolated_results = pandas.read_csv(
                os.path.normpath(os.path.join(out_dir, residue, interpolate_csv)),
                index_col=0,
            )

            interpolated_angles = interpolated_results.columns.values.astype(int)
            # Read in fit parameters
            fit_parameters = pandas.read_csv(
                os.path.normpath(os.path.join(out_dir, residue, fit_filename)),
                header=0,
                index_col=0,
            )
            for dataset in interpolated_results.index:
                ...
                np.testing.assert_almost_equal(
                    output, RMSD, decimal=7, err_msg="RMSD not calculated correctly"
                )

                RMSD_result = pandas.DataFrame(
                    data=RMSD, index=[dataset], columns=["RMSD"]
                )
...
def calculate_euclidean_distance(
    out_dir, ref_set, params, fit_type, datasets, subset=None
):

    """Calculates and store pairwise euclidean distances"""

    euclidean_base_csv = "euclidean_distances_from_{}_datasets_{}".format(
    ...
    for residue, data in ref_set.iterrows():
        ...
        if not os.path.exists(os.path.join(out_dir, residue, euclidean_csv)):
            ...
            euclidean_distance = pandas.DataFrame(
                index=fit_parameters.index, columns=fit_parameters.index
            )
            ...
            if subset == "Amplitudes":
                ...
                fit_parameters = fit_parameters[
                    ["a1", "a2", "a3", "mean_1", "mean_2", "mean_3"]
                ]
            else:
                logger.warning(
                    "Incorrect identifier for subset of parameters on which to perform clustering"
                )
            ...
            for dataset_col in fit_parameters.index:
                for dataset_row in fit_parameters.index:
                    euclidean_distance.loc[dataset_col][dataset_row] = euclidean(
                        fit_parameters.loc[dataset_col].values,
                        fit_parameters.loc[dataset_row].values,
                    )
            ...
            logger.info(
                "{}: Euclidean distance "
                "for {} datasets in "
                "{} seconds".format(residue, len(datasets), duration)
            )
        else:
            logger.info(
                "{}: Euclidean distance already calculated for these {} datasets".format(
