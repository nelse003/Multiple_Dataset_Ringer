...
from phil.multiple_dataset_phil import blank_arg_prepend, master_phil
...
def run(params):
    ...
    logger.info(
        "Choose a map_type: {}\n"
        "angle_type: {} \n"
        " by reducing reference set".format(
    ...
    for residue, data in ref_set.iterrows():
        ...
        single_residue_multiple_datasets = pd.read_csv(
            os.path.join(params.output.out_dir, residue, interpolate_csv), index_col=0
        )
        ...
        if not os.path.exists(
            os.path.join(
                ...
                residue,
                "all-{}-{}-dataset.png".format(residue, len(ref_set)),
            )
        ):

            multiple_line_plot_ringer(
                results_df=single_residue_multiple_datasets,
                title=residue,
                filename="all-{}-{}-dataset.png".format(residue, len(ref_set)),
    ...
    for residue, data in ref_set.iterrows():
        ...
        if not os.path.exists(
            os.path.join(params.output.out_dir, residue, correlation_csv)
        ):

            correlation_single_residue(
                input_csv=os.path.join(params.output.out_dir, residue, interpolate_csv),
    ...
    if not os.path.exists(
        os.path.join(
            ...
            "{}_ringer_{}_datasets.csv".format(average_type, len(datasets)),
        )
    ):

        average_ringer_plots(
            base_csv=interpolate_base_csv,
            ref_set=ref_set,
    ...
    min_corr, max_corr = find_pairwise_range(
        correlation_csv_end, ref_set, params.output.out_dir
    )
    ...
    if not os.path.exists(
        os.path.join(params.output.out_dir, "Modal_peak_location.png")
    ):

        max_peak_angle = []
        # Generate maximal values from interpolated map values
        for residue, data in ref_set.iterrows():
            interpolate_csv = "{}_{}_Datasets_{}_{}-ringer.csv".format(
                residue,
                len(datasets),
            ...
            angle_with_max_map = (interpolated_results.idxmax(axis=1).values).astype(
                numpy.float
            )
    ...
    fit_base_filename = "_from_{}_datasets_{}.csv".format(len(datasets), fit_type)
    ...
    ###################################################################
    euclidean_base_csv = calculate_euclidean_distance(
        params.output.out_dir,
        ref_set,
        params,
        fit_type,
    ...
    Adj_clusters_weight = pd.read_csv(
        os.path.join(
            params.output.out_dir,
            "Adj_clusters_weight_{}_{}_{}.csv".format(pairwise_type, fit_type, subset),
        ),
        header=0,
        index_col=0,
    )
    ...
    all_RMSD = pd.read_csv(
        os.path.join(params.output.out_dir, RMSD_filename), index_col=0, header=0
    )
    ...
    score_matrix = pd.DataFrame(index=residue_score.index, columns=dataset_score.index)

    for dataset in dataset_score.index:
        for residue in residue_score.index:
            score_matrix.loc[residue][dataset] = (dataset_score.loc[dataset]) * (
                residue_score.loc[residue]
            )
    ...
    plot_correlation_vs_fitting(
        params.output.out_dir, dataset_corr_score, dataset_score
    )
    ...
    plot_RMSD_vs_residue_score(
        params.output.out_dir, all_RMSD, residue_score, residue_means_score
    )
