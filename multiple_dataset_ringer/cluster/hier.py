...
# Imported functions
from multiple_dataset_ringer.plotting.plots import plot_dendrogram
from multiple_dataset_ringer.plotting.plots import number_clusters_histogram
...
def hier_agg_cluster(
    ...
    clusters_weight = pandas.DataFrame(index=ref_set.index.values, columns=datasets)
    ...
    for residue, data in ref_set.iterrows():
        ...
        dendrogram_filename = "{}_{}_{}_{}_dendrogram.png".format(
            residue, pairwise_type, fit_type, subset
        )
        ...
        if not os.path.exists(os.path.join(out_dir, residue, dendrogram_filename)):
            ...
            duration = end - start
            logger.info(
                "{}: Dendrogram ({}) generated in {} seconds.".format(
                    residue, pairwise_type, duration
                )
            )
        else:
            logger.info(
                "{}: Dendrogram already generated for {} "
        ...
        if not os.path.exists(os.path.join(out_dir, clusters_weight_filename)):
            ...
            for i in range(1, number_of_clusters + 1):
                ...
                for j in range(0, group_size):
                    clusters_weight.loc[residue:, grouped.groups[i][j]] = (
                        weight / number_of_clusters
                    )
        else:
            logger.info("Cluster weights ({}) already generated".format(pairwise_type))
        # Generate Heatmap

        heatmap_filename = "{}_{}_{}_{}_heatmap.png".format(
            residue, pairwise_type, fit_type, subset
        )
        if params.settings.gen_heatmap == True:
            if not os.path.exists(os.path.join(out_dir, residue, heatmap_filename)):
                ...
                logger.info(
                    "{}: Heatmap already generated for "
                    "{} with {} and {}".format(residue, pairwise_type, fit_type, subset)
                )
        else:
    ...
    if not os.path.exists(os.path.join(out_dir, cluster_number_hist)):
        if not num_cluster_all:
            ...
            number_clusters_histogram(num_cluster_all, cluster_number_hist, out_dir)
...
def find_pairwise_range(pairwise_csv_end, ref_set, out_dir):
    ...
    for residue, data in ref_set.iterrows():
        ...
        data = pandas.read_csv(
            os.path.join(out_dir, residue, pairwise_csv), index_col=0
        )
